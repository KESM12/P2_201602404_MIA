...
type MountNode struct {
	Key, Ruta   string
	Digits, Pos int
	Value       *estructuras.Partition
	ValueL      *estructuras.EBR
	Next, Prev  *MountNode
}

func (m *MountNode) MountNode(ruta string, digits int, pos int, value *estructuras.Partition, valueL *estructuras.EBR) {
	m.Ruta = ruta
	m.Digits = digits
	m.Pos = pos
	m.Value = value
...
func (m *MountNode) CreateKey() string {
	...
	return filename + strconv.Itoa(m.Pos) + "0" + strconv.Itoa(m.Digits)
}
...
func (m *MountList) Mount(path string, digit int, part *estructuras.Partition, partL *estructuras.EBR) {
	newNode := &MountNode{
		Ruta:   path,
		Key:    "",
		Digits: digit,
	...
	fmt.Println("Se ha montado la particion con id: " + newNode.Key)
	m.GetId(newNode)
}
...
func (m *MountList) PrintList() {
	...
	if !m.IsEmpty() {
		...
		for temp != nil {
			...
			if temp.Value != nil {
				contenido += fmt.Sprintf("%-15s", string(estructuras.TrimArray(temp.Value.Part_name[:])))
				...
				contenido += fmt.Sprintf("%-15s", string(estructuras.TrimArray(temp.ValueL.Part_name[:])))
